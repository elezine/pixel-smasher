...
for i in range(0, 30): # switch for testing # range(30): num_files
    name = dirpaths[i]
    ...
    results[i]=classify(os.path.join(sourcedir, name), os.path.join(outdir, name), thresh, None, method=method, og_mask_pth_in= os.path.join(sourcedir_R_mask, name_og_mask), water_index_type=water_index_type)
# pool.close()
# pool.join()
