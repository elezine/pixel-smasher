        ...
        if (plots_dir != None): # (res==SR) 
            ...
            if 1==0:
                ...
                for k, ires in enumerate(['HR', 'SR','Cub','LR']): # HR, SR, Cub, LR on btm says Katia #HERE <<-------------
                    ...
                    axs[k,2].imshow(tmp_output_XR_mask[k]>foreground_threshold, extent=xt, resample=False, cmap='Greys_r'), axs[0,2].set_title('Water mask', fontsize=fs+4), axs[k,2].set_axis_off() #axs[k,2].axes.yaxis.set_visible(False), axs[k,2].axes.xaxis.set_visible(False)
