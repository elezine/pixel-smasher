...
class ResidualDenseBlock_x10(nn.Module):
    ...
    def __init__(self, num_feat=80, num_grow_ch=40):
        super(ResidualDenseBlock_x10, self).__init__()
...
class RRDBNet_x10(nn.Module):
    ...
    def forward(self, x):
        ...
        # upsample
        feat = self.lrelu(
            self.conv_up1(F.interpolate(feat, scale_factor=5, mode='nearest')))
        feat = self.lrelu(
...
class ResidualDenseBlock_x4(nn.Module):
    """Residual Dense Block.
    ...
    def __init__(self, num_feat=64, num_grow_ch=32):
        super(ResidualDenseBlock_x4, self).__init__()
        self.conv1 = nn.Conv2d(num_feat, num_grow_ch, 3, 1, 1)
        self.conv2 = nn.Conv2d(num_feat + num_grow_ch, num_grow_ch, 3, 1, 1)
        self.conv3 = nn.Conv2d(num_feat + 2 * num_grow_ch, num_grow_ch, 3, 1,
                               1)
...
class RRDB_x4(nn.Module):
    ...
    def __init__(self, num_feat, num_grow_ch=40):
        super(RRDB_x4, self).__init__()
        self.rdb1 = ResidualDenseBlock_x4(num_feat, num_grow_ch)
        self.rdb2 = ResidualDenseBlock_x4(num_feat, num_grow_ch)
        self.rdb3 = ResidualDenseBlock_x4(num_feat, num_grow_ch)
...
class RRDBNet_x4(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_block=23,
                 num_grow_ch=32):
        super(RRDBNet_x4, self).__init__()
        self.conv_first = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)
        self.body = make_layer(
            RRDB_x4, num_block, num_feat=num_feat, num_grow_ch=num_grow_ch)
        self.conv_body = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        # upsample
        self.conv_up1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        self.conv_up2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
