...
'''
@weighted_loss
def ndwi_loss(pred, target, reduction='none'):
    ndwi_pred = (pred[:,0:1,:,:]-pred[:,2:3])/(pred[:,0:1,:,:]+pred[:,2:3])
    ndwi_target = (target[:,0:1,:,:]-target[:,2:3])/(target[:,0:1,:,:]+target[:,2:3])
    loss = ndwi_pres
    if reduction=='mean':
        
    return torch.sqrt((pred - target)**2 + eps)

class NDWILoss(nn.Module):
    """NDWI (normalized difference water index) loss.
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        """
        return self.loss_weight * ndwi_loss(
